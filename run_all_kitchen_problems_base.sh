...
process_observation() {
    ...
    SOLUTION="$SCRIPT_DIR/benchmarks/kitchen-100/02-solutions/$BASE_NAME.txt"
    ...
    "$SCRIPT_DIR/pandaPIpgrRepairVerify" pgrfo "domain-problem.psas" "solution.txt" "$OBS_COUNT" >> "$ABS_OBS_DIR/run.log" 2>&1
...
for PROBLEM_FILE in benchmarks/kitchen-100/01-problems/p-*.hddl; do
    ...
    PROBLEM_DIR="results_base/$BASE_NAME"
    mkdir -p "$PROBLEM_DIR"
