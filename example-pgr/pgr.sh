...
then
  ...
  do
    ...
    echo "$pgrprob"
    # run planner and save per-PGR log (so we don't overwrite one single log)
    ./pplanner "$pgrprob" 2>&1 | tee "${pgrprob}.log"
    # create a human-readable plan from the per-PGR log
    ./pandaPIparser -c "${pgrprob}.log" "${pgrprob}.plan"
  done
