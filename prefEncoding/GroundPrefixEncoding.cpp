...
void GroundPrefixEncoding::prefixEncoding(Model *htn, string sasPlan, encodingType enc, bool techVisible, int stopAfter) {
    ...
    for (int t : typeTactions) {
        int prec = -1;
        // For fully observable PGR and Repair: actions must come after prefix
        // For partially observable PGR: actions remain unchanged (no prefix constraint)
        ...
        // Note: PGRpo case deliberately omitted - no precondition added
    ...
    if (writeDummy) {
        ...
        // Debug output for diagnosing task count mismatches
        cout << "- debug: tdReachableT.size()=" << tdReachableT.size()
             << " distPrefActions.size()=" << distPrefActions.size()
             << " prefix.size()=" << prefix.size()
             << " writeDummy=" << writeDummy << endl;
        cout << "- debug: numTasks=" << numTasks << endl;
        fOut << numTasks << endl;
